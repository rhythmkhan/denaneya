...
export class NagadStatementParser implements StatementParser {
  ...
  async parse(content: string, batchReference?: string): Promise<StatementBatch> {
    ...
    if (isJson) {
      try {
        ...
        throw new StatementParsingError(`Failed to parse Nagad JSON statement: ${(err as Error).message}`);
      }
    } else {
    ...
    for (let index = 0; index < rawRows.length; index++) {
      ...
      const orderId = String(row.orderid ?? row.orderId ?? row.order_id ?? row.merchanttxid ?? '').trim();
      const paymentRefId = String(row.paymentrefid ?? row.paymentRefId ?? row.payment_ref_id ?? row.providertrxid ?? '').trim();
      const amountStr = String(row.amount ?? '0').trim();
      const chargeStr = String(row.charge ?? row.fee ?? '0.00').trim();
      const rawStatus = String(row.status ?? '').trim().toUpperCase();
      const timeStr = String(row.datetime ?? row.dateTime ?? row.date_time ?? row.transactionTime ?? '').trim();
      ...
      try {
        ...
        feePaisa = chargeStr ? Paisa.fromBDT(chargeStr).toPaisa() : 0n;
      } catch (err) {
        throw new StatementParsingError(
          `Invalid amount in Nagad statement at row ${index + 1}: ${(err as Error).message}`,
          { row, index }
        );
      }
      ...
      const item: StatementItem = {
        ...
        providerTrxId: paymentRefId || orderId,
        merchantTxId: orderId || undefined,
        amountPaisa,
        feePaisa,
        netAmountPaisa,
        currency: 'BDT',
    ...
    const ref = batchReference ?? `NAGAD-${statementDate.toISOString().slice(0, 10)}-${fileHash.slice(0, 8)}`;
